...
public class GameFlowManagerForNet : NetworkBehaviour
{
    [Header("Parameters")]
    ...
    public ArcadeKartForNet playerKart;

    ArcadeKartForNet[] karts;
    ...
    void Start()
    {
        if (autoFindKarts)
        {
            karts = FindObjectsOfType<ArcadeKartForNet>();
            if (karts.Length > 0)
            {
                if (!playerKart) playerKart = karts[0];
            }
            //DebugUtility.HandleErrorIfNullFindObject<ArcadeKartForNet, GameFlowManager>(playerKart, this);
        }
        ...
        foreach (ArcadeKartForNet k in karts)
        {
			k.SetCanMove(false);
        }
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if ((m_TimeManager.IsFinite && m_TimeManager.IsOver) || (m_Finish && !m_Win))
                EndGame(false);
        }
    }
