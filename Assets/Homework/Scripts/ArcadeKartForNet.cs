...
{
    public class ArcadeKartForNet : NetworkBehaviour
    {
        /// <summary>
        ...
        {
            public ArcadeKartForNet.Stats modifiers;
        ...
        [SyncVar]
        public ArcadeKartForNet.Stats baseStats = new ArcadeKartForNet.Stats
        {
            TopSpeed            = 10f,
            Acceleration        = 5f,
        ...
        [SyncVar]
        ArcadeKartForNet.Stats finalStats;

        public override void OnStartLocalPlayer()
        {
            GameObject cvc = GameObject.Find("CinemachineVirtualCamera");
            if (cvc != null)
            {
                cvc.GetComponent<CinemachineVirtualCamera>().Follow = transform;
                cvc.GetComponent<CinemachineVirtualCamera>().LookAt = transform;
            }

            GameObject gm = GameObject.Find("GameManager");
            if (gm != null)
                gm.GetComponent<GameFlowManagerForNet>().playerKart = this;

            GetComponent<ChangeLocalPlayerColor>().ChangeColor();
        }
        ...
        void FixedUpdate()
        {
            ...
            CmdGroundVehicle(m_MinHeight);
            if (canMove)
            {
                CmdMoveVehicle(accel, turn);
            }
            CmdGroundAirbourne();

            // animation
            CmdAnimateSuspension();
        }

        [Command]
        private void CmdChangeCenterOfMass()
        {
        ...
        [Command]
        private void CmdCalculatePercent()
        {
        ...
        [Command]
        void CmdGroundVehicle(float minHeight)
        {
            if (GroundPercent >= 1f)
            {
        ...
        [Command]
        void CmdMoveVehicle(float accelInput, float turnInput)
        {
